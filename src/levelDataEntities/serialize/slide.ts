...
export const serializeSlidesToLevelDataEntities = (
  ...
  const allowSimLines = new Map<number, LevelDataEntity[]>()
  ...
  for (const infos of store.slides.info.values()) {
    ...
    for (const [i, info] of infos.entries()) {
      ...
      const isInActive = info.attachHead !== info.attachTail
      const isActiveHead = info.attachHead === info.note
      const isActiveTail = info.attachTail === info.note
      ...
      if (!isFirst) {
        ...
        entity.archetype += info.note.noteType === "default" ? "Tap" : info.note.noteType.split("").map((c, i) => (i ? c : c.toUpperCase())).join("")
      }
      // }
      ...
      if (
        ...
        if (notes) {
          ...
          allowSimLines.set(info.note.beat, [entity])
        }
      }
