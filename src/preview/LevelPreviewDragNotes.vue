...
import { noteDuration, dragNoteLayout, approach } from './note'
import { Quad } from "./Quad"

const dragNotes = computed(() =>
    [...cullEntities('dragNote', keys.value.min, keys.value.max)]
...
<template>
    <g stroke="white" stroke-width="0">
        <polygon v-for="(polygon, index) in dragNotes" :key="index" v-bind="polygon" />
    </g>
</template>
